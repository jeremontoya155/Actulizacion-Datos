...
router.post('/buscar', async (req, res) => {
    ...
    try {
        ...
        if (resultPostgre.rows.length > 0) {
            // Si el cliente existe en PostgreSQL, mostrar los datos y dar el aviso
        ...
        originalDB.query('SELECT CodCliente, Nombre, Documento, Telefono, Email, Domicilio FROM clientes WHERE Documento = ?', [dni], (err, results) => {
            ...
            if (results.length > 0) {
                // Si el cliente existe en Plex, rellenar el formulario con esos datos
                const cliente = {
                ...
                return res.render('formulario', { cliente, editar: false, dni, message: null });
            }

            // 3. Si no está en PostgreSQL ni en Plex, mostrar formulario vacío
            return res.render('formulario', { cliente: {}, editar: false, dni, message: null });
        });
    } catch (error) {
